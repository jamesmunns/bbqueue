...
#[cfg(test)]
mod tests {
    use bbqueue::{BBQueue, Error as BBQError, OwnedBBBuffer as Obbb};

    #[test]
    fn deref_deref_mut() {
        let bb: BBQueue<Obbb<6>> = BBQueue::new(Obbb::new());
    ...
    fn static_allocator() {
        ...
        static BBQ1: BBQueue<Obbb<6>> = BBQueue::new(Obbb::new());
        static BBQ2: BBQueue<Obbb<6>> = BBQueue::new(Obbb::new());
    ...
    // fn release() {
    //     // Check we can make multiple static items...
    //     static BBQ1: BBQueue<6> = BBQueue::new();
    //     static BBQ2: BBQueue<6> = BBQueue::new();
    ...
    // #[test]
    // fn zero_sized_grant() {
    //     let bb: BBQueue<1000> = BBQueue::new();
    ...
    // #[test]
    // fn frame_big_little() {
    //     let bb: BBQueue<65536> = BBQueue::new();
    //     let (mut prod, mut cons) = bb.try_split_framed().unwrap();
